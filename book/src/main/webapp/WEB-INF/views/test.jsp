...
function purchase(){
	if(confirm("구매하시겠습니까?")){
		location.href="./purchase?tno=${tno }";
	}
}
...
$(".pr-remove").click(function(){
	 if(confirm("취소하시겠습니까?")){
		 ...
		 //let cartno = $(this).closest("tr").find(".cartno").text();
	 	let cartno = $(this).parents(".seed").siblings(".cartno").text();
	 	let tree = $(this).parents(".tree");
	    let mod = $(this).siblings(".mod").val();
	    let amount = $(this).parents(".seed").siblings(".amount2").text();
...
/*  function del(cartno) {
	...
	//if(confirm("취소하시겠습니까?")){
    //location.href="./delete?cartno="+cartno;
									...
									<c:forEach items="${cart}" var="row">
										...
										<c:if
											...
											<tr class="tree">
												...
												<td class="hidden-xs">
													<h5 class="product-title font-alt">${row.price}원</h5>
												</td>
												...
												<td class="seed"><input class="mod" type="number" name="mod" min="1" max="${row.amount }"><button class="pr-remove" type="button">담기 취소</button></td>
						...
						<div class="col-sm-5 col-sm-offset-7">
							<div class="shop-Cart-totalbox">
								...
								<button class="btn btn-lg btn-block btn-round btn-d"
									type="submit" onclick="purchase()">결제하기</button>
							</div>
						</div>
					</div>
				</div>
