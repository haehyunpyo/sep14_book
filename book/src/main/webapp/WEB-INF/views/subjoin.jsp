						...
						<div>
							<c:choose>
           						<c:when test="${mname ne null}">
           							<input type="text" name="mname" id="mname" value="${mname }" disabled/>
           						</c:when>
           						<c:otherwise>
					...
					<div>
						...
						<div class="">
							<c:choose>
								...
								<c:otherwise>
									<c:choose>
										<c:when test="${sessionScope.withN eq 2}"><span>네이버 계정이 올바른지 확인필요</span></c:when>
										<c:when test="${sessionScope.withK eq 1}"><span>카카오 계정이 올바른지 확인필요</span></c:when>
										<c:otherwise>
											<input type="text" name="memail" id="memail" value="${memail }" disabled/>
